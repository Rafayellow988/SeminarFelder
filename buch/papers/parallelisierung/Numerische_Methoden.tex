...
Als Ausgangslage wird die allgemeine Wärmeleitungsgleichung betrachtet als
\begin{equation}
...
Die partielle Ableitung der Temperaturfunktion \( T \) nach der Zeit wird durch eine explizite Vorwärtsdifferenz approximiert mit
...
Auch hier beginen wir mit der einmaligen Anwendungen des Diefferentielquotientsen (Vorwärts), um die erste Ableitung, zunächst in x-Richtung zu approximieren verwenden wir
...
\end{equation}
um mittels einer letzen Anwendung des Differentialquotienten die Änderung dieser beiden Änderungen, also genau das was die zweite Ableitung beschreibt, diskret zu bestimmen als
\begin{equation}
...
Nun setzen wir die in (\ref{parallelisierung:eq:discrete_time_derivative}), (\ref{parallelisierung:eq:discrete_x_derivative}) und (\ref{parallelisierung:eq:discrete_y_derivative}) gefundenen Gleichungen in die Ursprüngliche zweidimensionale Wärmeleitungsgleichung (\ref{parallelisierung:eq:Wärmeleitung_2D})ein.
Somit erhalten wir 
...
Aufgrund der Lokalität und Kausalität der Feldgleichungen ist die Temperatur eines Gitterpunkts \( (x_i, y_j)\) zu einem bestimmten Zeitpunkt in der Zukunft \( (t^n)\) sprich \(T_{i,j}^{n+1}\) nur von dem  vergangenen Zustand eben dieses Gitterpunkts und dessen direkten Nachbarn abhängig.
Dies ist  in der nach \(T_{i,j}^{n+1}\) umgestellten Formel (\ref{parallelisierung:eq:update_formula_unsorted}) 
\begin{equation}
...
Nun sind wir in der Lage Formel \ref{parallelisierung:eq:update_formula_sorted} kompakt und übersichtlich zu schreiben als
...
\begin{equation}
	...
	\right).
\end{equation}
Diese Formel wird als explizite update Formel bezeichnet, da sie iterativ auf Grundlage von gemessenen Daten oder zuvor berechneten Datenpunkten den nächsten Zeitschritt auswertet.
...
\begin{equation}
	...
	\frac{\alpha \, \Delta t}{(\Delta y)^2} \leq \frac{1}{2}.
\end{equation}
Mit \(\Delta l = \Delta x = \Delta y\) folgt \(\lambda = \lambda_x = \lambda_y\), sodass sich die Formel ergibt zu
\begin{equation}
...
\begin{center}
	...
	\end{tabular}
\end{center}
Somit ergibt sich die thermische Diffusität wie folgt zu
...
\approx 0.376 > \frac14.
\]
Zunächst scheint die Simulation noch plausibel (Abbildung~\ref{parallelisierung:fig:simulation_15x15_0.376_4it}), doch bereits nach wenigen Iterationen treten unphysikalische Werte auf (Abbildung~\ref{parallelisierung:fig:simulation_15x15_0.376_6it}), wie etwa Temperaturabfälle an Stellen, an denen ein Anstieg zu erwarten wäre.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{papers/parallelisierung/images/simulation_15x15_0.376_4it.pdf}
	\caption{\(15\times 15\)-Gitter, \(\lambda = 0.376\), nach 4 Iterationen.}
	\label{parallelisierung:fig:simulation_15x15_0.376_4it}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{papers/parallelisierung/images/simulation_15x15_0.376_6it.pdf}
	\caption{\(15\times 15\)-Gitter, \(\lambda = 0.376\), nach 6 Iterationen: erste Instabilitätsartefakte.}
	\label{parallelisierung:fig:simulation_15x15_0.376_6it}
\end{figure}
...
\begin{equation}
	...
	\right).
\end{equation}
Für \(\lambda > \tfrac14\) wird der Koeffizient \((1-4\lambda)\) negativ — das Zentrum wird „negativ gewichtet“.  
