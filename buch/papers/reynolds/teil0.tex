...
man das Ziel, sich auf Phänomene zu fokusieren, die in etwa eine Zeitspanne $\Delta t$ überdauern.
Zeitlich schnellere Phänomene werden über eine Zeit von $\Delta t$ gemittelt.

Im ersten Schrit werden die Unbekannten geschwindigkeiten $u_\cdot$ und der druck $p$ in den Navier-Stokes-Gleichungen \ref{reynolds:eqs:impulse}
...
\subsection{Beipiel anhand von k-$\epsilon$}
...
Mit den beiden Feldern k und $\epsilon$ kann die Turbulente Viskosität (auch wider ein Skalarfeld) wie folgt errechnte werden.
...
Der Wert der Turbulente Viskosität ist dort am höchsten wo auch die meisten Turbulenzen auftreten.
Der in ~\ref{fig:mu-t} ist es ein sehr anschauliches Beispiel,
da man genau an den Orten mit der höchsten Werten der Turbulenten Viskosität die vollständige ablösung der Grenzschicht erwarten würde.
(Im Breich nach der Ablösung der Grenzschicht entstehen die meisten Turbulenzen).

Dammit könne alle sechs Therme des Reynoldsspannungstensor folgendermasen berechnete werden:
Die Diagonalen Therme $R_{11}$, $R_{22}$ und $R_{33}$:
\begin{equation}
    R_{ii} = 2 \mu_t \frac{\partial u_i}{\partial x_i} - \frac{2}{3}\rho k
\end{equation}
Und den Thermen $R_{12} = R_{21}$, $R_{13} = R_{31}$ und $R_{23} = R_{32}$
\begin{equation}
