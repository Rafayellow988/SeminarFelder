...
\begin{figure}
	...
	\begin{tikzpicture}[scale=1.05, >=stealth]
		...
		\draw (0,0) ellipse ({\R1} and {\r});
		\draw (-\R,0) -- (-\R1,\H);
		\draw (\R1,0) -- (\R1,\H);
		\draw (0,\H) ellipse ({\R1} and {\r});
		\node[above] at (0,\H+0.35) {$\zeta$};
		\draw[->] (1.4,1.7) -- (0.7,1.7);

		% right cylinder (H+dH)
		\begin{scope}[xshift=\DX cm]
			\draw (0,{-\dH/2}) ellipse ({\R2} and {\r});         % bottom
			\draw (-\R2,{-\dH/2}) -- (-\R2,{\H+\dH/2});
			\draw ( \R2,{-\dH/2}) -- ( \R2,{\H+\dH/2});
			\draw (0,{\H+\dH/2}) ellipse ({\R2} and {\r});        % top
			\node[above] at (0,{\H+\dH/2+0.35}) {$\zeta+\mathrm{d}\zeta$};
			\node[right] at (\R2+0.25,{\H/2}) {$H+\mathrm{d}H$};  % center the label
			\draw[->] (-1.4,{\H/2}) -- (-0.7,{\H/2});
		\end{scope}


		\begin{scope}[xshift={-\DX cm}]
			\draw (0,{ \dH/2}) ellipse ({\R3} and {\r});          % bottom
			\draw (-\R3,{ \dH/2}) -- (-\R3,{\H-\dH/2});
			\draw ( \R3,{ \dH/2}) -- ( \R3,{\H-\dH/2});
			\draw (0,{\H-\dH/2}) ellipse ({\R3} and {\r});        % top
			\node[above] at (0,{\H-\dH/2+0.35}) {$\zeta-\mathrm{d}\zeta$};
			\node[left]  at (-\R3-0.25,{\H/2}) {$H-\mathrm{d}H$}; % center the label
		\end{scope}
		% spin symbols on tops
		% spin symbols: use H±dH/2 so the midline matches
		\foreach \x/\y in {-\DX/{\H-\dH/2}, 0/{\H}, \DX/{\H+\dH/2}}{
