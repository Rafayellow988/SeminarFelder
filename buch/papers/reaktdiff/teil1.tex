...
Im Fokus liegt die Stationärlösung \(u_0, v_0\) der Gleichungen \eqref{reaktdiff:equation:reaktdiff2ohneDiff}, dass heisst das \(f(u_0,v_0) = g(u_0,v_0) = 0\).
...
\begin{align*}
    \text{Tr}(J) &= f_u + g_v < 0,
    %\label{reaktdiff:equation:reaktdiffbedingung1} 
    \\
    \det(J) &= f_u g_v - f_v g_u > 0.
    %\label{reaktdiff:equation:reaktdiffbedingung2}
\end{align*}
...
\begin{figure}
    ...
    \caption{FFT der Animation. Die rote Linie zeigt die theortische minimale Frequnez von \(f = k_{\min} / 2 \pi \approx 0.109\). Man sieht, dass die dominanten Frequenzen links des berechneten Minimums ist.}
